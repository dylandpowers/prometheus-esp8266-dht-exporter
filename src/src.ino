...
char str_temp[6], str_hum[6], str_heat[6];
...
void setup_http_server() {
  ...
  http_server.on("/temperature", HTTPMethod::HTTP_GET, handle_http_temperature);
  http_server.on("/humidity", HTTPMethod::HTTP_GET, handle_http_humidity);
  http_server.on("/heatIndex", HTTPMethod::HTTP_GET, handle_http_heat_index);
...
void handle_http_metrics() {
  ...
  static char const *response_template =
    ...
    PROM_NAMESPACE "_air_humidity_percent %s\n"
    "# HELP " PROM_NAMESPACE "_air_temperaturel_fahrenheit Air temperature.\n"
    "# TYPE " PROM_NAMESPACE "_air_temperature_fahrenheit gauge\n"
    "# UNIT " PROM_NAMESPACE "_air_temperature_fahrenheit \u00B0C\n"
    PROM_NAMESPACE "_air_temperature_fahrenheit %s\n"
    "# HELP " PROM_NAMESPACE "_air_heat_index_fahrenheit Apparent air temperature, based on temperature and humidity.\n"
    "# TYPE " PROM_NAMESPACE "_air_heat_index_fahrenheit gauge\n"
    "# UNIT " PROM_NAMESPACE "_air_heat_index_fahrenheit \u00B0C\n"
    PROM_NAMESPACE "_air_heat_index_fahrenheit %s\n";
  ...
  char response[BUFSIZE];
  snprintf(response, BUFSIZE, response_template, VERSION, BOARD_NAME, DHT_NAME, str_hum, str_temp, str_heat);
  http_server.send(200, "text/plain; charset=utf-8", response);
}

void handle_http_temperature() {
  read_temperature_sensor();
  if (isnan(temperature)) {
    http_server.send(500, "text/plain; charset=utf-8", "Sensor error.");
  }

  http_server.send(200, "text/plain; charset=utf-8", str_temp);
}

void handle_http_humidity() {
  read_humidity_sensor();
  if (isnan(humidity)) {
    http_server.send(500, "text/plain; charset=utf-8", "Sensor error.");
  }

  http_server.send(200, "text/plain; charset=utf-8", str_hum);
}


void handle_http_heat_index() {
  read_heat_index();
  if (isnan(heat_index)) {
    http_server.send(500, "text/plain; charset=utf-8", "Sensor error.");
  }

  http_server.send(200, "text/plain; charset=utf-8", str_heat);
}
...
void read_sensors(boolean force) {
  uint32_t min_interval = std::max(dht_sensor.getMinimumSamplingPeriod(), READ_INTERVAL);
...
void read_temperature_sensor() {
  ...
  dtostrf(celsius_to_fahrenheit(temperature), 4, 2, str_temp);
}
...
void read_heat_index() {
  ...
  dtostrf(celsius_to_fahrenheit(heat_index), 4, 2, str_heat);
}
...
void log(char const *message, LogLevel level) {
  ...
  char record[150];
  snprintf(record, 150, "[%s] %s", str_level, message);
